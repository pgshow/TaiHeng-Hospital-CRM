...
<input type="hidden" name="http_referer" value="<?php echo $_SERVER['HTTP_REFERER']; ?>" /><table class="form-table">
    ...
    <select name="province" id="province" onChange="loadArea(this.value,'city')">
        ...
        <?php if(is_array($province)): $i = 0; $__LIST__ = $province;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i;?><option value="<?php echo ($one["area_id"]); ?>"><?php echo ($one["area_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
    </select>
